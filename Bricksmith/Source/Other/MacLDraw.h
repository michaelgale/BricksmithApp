...
{
    MouseDraggingOff             = 0, MouseDraggingBeginImmediately                     = 1,
    MouseDraggingBeginAfterDelay = 2, MouseDraggingImmediatelyInOrthoNeverInPerspective = 3
} MouseDragBehaviorT;

typedef enum RightButtonBehavior
{
    RightButtonContextual = 0, RightButtonRotates    = 1
} RightButtonBehaviorT;

typedef enum RotateMode
{
    RotateModeTrackball = 0, RotateModeTurntable = 1
} RotateModeT;

typedef enum MouseWheelBeahvior
{
    MouseWheelScrolls = 0, MouseWheelZooms   = 1
} MouseWheelBeahviorT;

typedef enum PartBrowserStyle
{
    PartBrowserShowAsDrawer = 0, PartBrowserShowAsPanel  = 1
} PartBrowserStyleT;


typedef enum SearchMode
{
    SearchModeAllCategories = 0, SearchModeSelectedCategory = 1
} SearchModeT;
