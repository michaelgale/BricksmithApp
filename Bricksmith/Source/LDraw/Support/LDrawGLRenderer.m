...
- (void)setBackgroundColorRed:(float)red green:(float)green blue:(float)blue
{
    ...
    glClearColor(glBackgroundColor[0], glBackgroundColor[1], glBackgroundColor[2], glBackgroundColor[3]);
...
- (void)setLDrawDirective:(LDrawDirective *)newFile
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:LDrawDirectiveDidChangeNotification object:
    nil];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:LDrawFileActiveModelDidChangeNotification
    object:nil];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:
    LDrawModelRotationCenterDidChangeNotification object:nil];

    if (self->fileBeingDrawn != nil) {
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(displayNeedsUpdating:)name:
        LDrawDirectiveDidChangeNotification object:self->fileBeingDrawn];

        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(activeModelDidChange:)name:
        LDrawFileActiveModelDidChangeNotification object:self->fileBeingDrawn];
...
- (void)moveCamera:(Vector3)delta
{
    ...
    Vector3 invertedDelta = V3Make(mv[0] * delta.x + mv[4] * delta.y + mv[8] * delta.z,
...
- (BOOL)mouseSelectionClick:(Point2)point_view selectionMode:(SelectionModeT)selectionMode
{
    ...
    // select it.
    if (self->fileBeingDrawn != nil && self->allowsEditing == YES &&
        ...
        Matrix4 mvp = Matrix4Multiply(Matrix4CreateFromGLMatrix4([camera getModelView]),
                Matrix4CreateFromGLMatrix4([camera getProjection]));

        id bestObject = nil;
        [fileBeingDrawn depthTest:point_clip inBox:test_box transform:mvp creditObject:nil bestObject:&
        bestObject bestDepth:&depth];
            ...
            switch (selectionMode)
            {
                case SelectionReplace :
                    ...
                    if (!has_sel_directive) {
                        [self->delegate LDrawGLRenderer:self wantsToSelectDirective:clickedDirective
...
- (void)dragHandleDraggedToPoint:(Point2)point_view constrainDragAxis:(BOOL)constrainDragAxis
{
    ...
    moved =
        [self updateDirectives:[NSArray arrayWithObject:self->activeDragHandle] withDragPosition:point_view
        depthReferencePoint:modelReferencePoint constrainAxis:constrainDragAxis];
...
- (void)mouseSelectionDragToPoint:(Point2)point_view selectionMode:(SelectionModeT)selectionMode
{
    ...
    if (self->fileBeingDrawn != nil && self->allowsEditing == YES &&
        ...
        fineDrawParts =
            [self getDirectivesUnderRect:self->selectionMarquee amongDirectives:[NSArray arrayWithObject:self
            ->fileBeingDrawn] fastDraw:NO];
        [self->delegate LDrawGLRenderer:self wantsToSelectDirectives:fineDrawParts selectionMode:selectionMode];
    }
    ...
    setTransform originatedLocally:(BOOL)originatedLocally
{
    ...
    // Move the parts
    [self updateDirectives:directives withDragPosition:point_view depthReferencePoint:modelReferencePoint
    constrainAxis:NO];
...
- (void)updateDragWithPosition:(Point2)point_view constrainAxis:(BOOL)constrainAxis
{
    ...
    if ([self->fileBeingDrawn respondsToSelector:@selector(draggingDirectives)]) {
        ...
        moved =
            [self updateDirectives:directives withDragPosition:point_view depthReferencePoint:
            modelReferencePoint constrainAxis:constrainAxis];
        if (moved) {
            [self->fileBeingDrawn noteNeedsDisplay];
        }
    }
...
- (double)getDepthUnderPoint:(Point2)point_view
{
    ...
    [fileBeingDrawn depthTest:point_clip inBox:test_box transform:mvp creditObject:nil bestObject:&bestObject
    bestDepth:&depth];
    ...
    fastDraw
{
    ...
    if ([directives count] == 0) {
        ...
        // Do hit test
        for (counter = 0; counter < [directives count]; counter++) {
            [[directives objectAtIndex:counter] boxTest:test_box transform:mvp boundsOnly:fastDraw
...
- (Point3)modelPointForPoint:(Point2)viewPoint
{
    ...
    if (depth == 1.0) {
        ...
        modelPoint = [self modelPointForPoint:viewPoint depthReferencePoint:partTransform.translate];
    }
    else {
        // Convert to 3D viewport coordinates
