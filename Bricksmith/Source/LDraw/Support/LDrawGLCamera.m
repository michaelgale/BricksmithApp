...
- (Box2)nearFrustumClippingRectFromVisibleRect:(Box2)visibleRectIn
{
    ...
    float visibleProportion = (fabs(self->cameraDistance) - fieldDepth / 2) / fabs(self->cameraDistance);
...
- (Box2)visibleRectFromNearOrthoClippingRect:(Box2)visibilityPlane
{
    ...
    if (1) { // [self isFlipped] == YES)
        newVisibleRect.origin.y = _graphicsSurfaceSize.height - V2BoxHeight(visibilityPlane) -
...
- (void)tickle
{
    ...
    if (scroller) {
        ...
        Size2 expandedViewportSize = V2MakeSize(MAX(fabs(V2BoxMinX(self.visibleRect) -
                V2BoxMidX(viewportRect)),
                fabs(V2BoxMaxX(self.visibleRect) - V2BoxMidX(viewportRect))) * 2,
                MAX(fabs(V2BoxMinY(self.visibleRect) - V2BoxMidY(viewportRect)),
                fabs(V2BoxMaxY(self.visibleRect) - V2BoxMidY(viewportRect))) * 2);
        ...
        newDocumentRect.size.width = MAX(_graphicsSurfaceSize.width, MAX(snugDocumentSize.width,
                expandedViewportSize.width));
        newDocumentRect.size.height = MAX(_graphicsSurfaceSize.height, MAX(snugDocumentSize.height,
...
- (void)rotationDragged:(Vector2)viewDirection
{
    ...
    applyRotationMatrix(orientation,
        rotationAboutX,
        transformedVectorX.x,
        transformedVectorX.y,
        transformedVectorX.z);
    applyRotationMatrix(orientation,
        rotationAboutY,
        transformedVectorY.x,
        transformedVectorY.y,
        transformedVectorY.z);
    [self makeModelView];
} // end rotationDragged
