...
struct LDrawDLBuilder *LDrawDLBuilderCreate(void)
{
    ...
    struct LDrawDLBuilderPerTex *untex = (struct LDrawDLBuilderPerTex *)LDrawBDPAllocate(alloc,
            sizeof(struct LDrawDLBuilderPerTex));

    memset(untex, 0, sizeof(struct LDrawDLBuilderPerTex));

    struct LDrawDLBuilder *bld = (struct LDrawDLBuilder *)LDrawBDPAllocate(alloc,
            sizeof(struct LDrawDLBuilder));
...
void LDrawDLBuilderAddLine(struct LDrawDLBuilder *ctx, const GLfloat v[6], GLfloat n[3], GLfloat c[4])
{
    ...
    int i;
    struct LDrawDLBuilderVertexLink *nl = (struct LDrawDLBuilderVertexLink *)LDrawBDPAllocate(ctx->alloc,
            sizeof(struct LDrawDLBuilderVertexLink) + sizeof(GLfloat) * VERT_STRIDE * 2);
    nl->next   = NULL;
    nl->vcount = 2;
    for (i = 0; i < 2; ++i) {
        copy_vec3(nl->data + VERT_STRIDE * i, v + i * 3);
...
struct LDrawDL *LDrawDLBuilderFinish(struct LDrawDLBuilder *ctx)
{
    ...
    struct LDrawDL *dl = (struct LDrawDL *)malloc(sizeof(struct LDrawDL) + sizeof(struct LDrawDLPerTex) *
            total_texes);
    ...
    for (s = ctx->head; s; s = s->next) {
        ...
        for (l = s->tri_head; l; l = l->next) {
            add_face(M, l->data, l->data + 10, l->data + 20, NULL, l->data + 6, ti);
        }
...
{
    struct LDrawBDP *alloc = LDrawBDPCreate();
    struct LDrawDLSession *session = (struct LDrawDLSession *)LDrawBDPAllocate(alloc,
            sizeof(struct LDrawDLSession));
...
void LDrawDLSessionDrawAndDestroy(struct LDrawDLSession *session)
{
    ...
    if (session->dl_head) {
        ...
        if (segments != cur_segment) {
            ...
            for (s = segments; s < cur_segment; ++s) {
                ...
                if (s->dl->quad_count) {
                    glDrawArraysInstancedARB(GL_QUADS,
                        s->dl->quad_off,
                        s->dl->quad_count,
                        s->inst_count);
                }
    ...
    if (session->sorted_head) {
        ...
        for (lc = 0; lc < session->sort_count; ++lc) {
            int i;
            for (i = 0; i < 4; ++i) {
                glVertexAttrib4f(attr_transform_x + i, l->transform[i], l->transform[4 + i],
