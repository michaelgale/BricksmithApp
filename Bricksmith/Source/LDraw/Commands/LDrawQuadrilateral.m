...
- (id)initWithLines:(NSArray *)lines inRange:(NSRange)range parentGroup:(dispatch_group_t)parentGroup
{
    ...
    @try
    {
        ...
        if ([parsedField integerValue] == 4) {
            ...
            @throw [NSException exceptionWithName:@"BricksmithParseException" reason:@"Bad quad syntax"
                   userInfo:nil];
        }
    }
...
- (void)encodeWithCoder:(NSCoder *)encoder
{
    ...
    [encoder encodeBytes:(void *)&vertex1 length:sizeof(Point3)
    forKey:@"vertex1"];
    [encoder encodeBytes:(void *)&vertex2 length:sizeof(Point3)
    forKey:@"vertex2"];
    [encoder encodeBytes:(void *)&vertex3 length:sizeof(Point3)
    forKey:@"vertex3"];
    [encoder encodeBytes:(void *)&vertex4 length:sizeof(Point3)
    forKey:@"vertex4"];
    [encoder encodeBytes:(void *)&normal length:sizeof(Vector3)
    forKey:@"normal"];
} // end encodeWithCoder:
...
- (void)collectSelf:(id <LDrawCollector>)renderer
{
    ...
    if (self->hidden == NO) {
        GLfloat v[12] =
        {
            vertex1.x, vertex1.y, vertex1.z, vertex2.x, vertex2.y, vertex2.z,     vertex3.x,     vertex3.y,
            vertex3.z,
    ...
    creditObject hits:(NSMutableSet *)hits
{
    if (self->hidden == NO) {
        ...
        float h_tri2[12] =
        {
            clipVertex3.x, clipVertex3.y, clipVertex3.z, clipVertex3.w, clipVertex4.x, clipVertex4.y,
            clipVertex4.z, clipVertex4.w, clipVertex1.x, clipVertex1.y, clipVertex1.z, clipVertex1.w
        };
...
- (NSString *)write
{
    return([NSString stringWithFormat:@"4 %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@",
           ...
           [LDrawUtilities outputStringForFloat:vertex4.x], [LDrawUtilities outputStringForFloat:vertex4.y],
    ...
    drawingColor wireframe:(BOOL)wireframe
{
    ...
    if (wireframe == NO) {
        ...
        vertexBuffer[3].position[0] = (GLfloat)vertex4.x;
        vertexBuffer[3].position[1] = (GLfloat)vertex4.y;
        vertexBuffer[3].position[2] = (GLfloat)vertex4.z;
...
// memcpy(&vertexBuffer[3].color,    components,	sizeof(GLfloat)*4);
        ...
        memcpy(&vertexBuffer[4].color, components, sizeof(GLfloat) * 4);
        vertexBuffer[5].position[0] = (GLfloat)vertex4.x;
        vertexBuffer[5].position[1] = (GLfloat)vertex4.y;
        vertexBuffer[5].position[2] = (GLfloat)vertex4.z;
...
//
        // edge4
        vertexBuffer[6].position[0] = (GLfloat)vertex4.x;
        vertexBuffer[6].position[1] = (GLfloat)vertex4.y;
        vertexBuffer[6].position[2] = (GLfloat)vertex4.z;
        vertexBuffer[6].normal[0]   = (GLfloat)normal.x;
        vertexBuffer[6].normal[1]   = (GLfloat)normal.y;
        vertexBuffer[6].normal[2]   = (GLfloat)normal.z;
        memcpy(&vertexBuffer[6].color, components, sizeof(GLfloat) * 4);
        vertexBuffer[7].position[0] = (GLfloat)vertex1.x;
        vertexBuffer[7].position[1] = (GLfloat)vertex1.y;
        vertexBuffer[7].position[2] = (GLfloat)vertex1.z;
        vertexBuffer[7].normal[0]   = (GLfloat)normal.x;
        vertexBuffer[7].normal[1]   = (GLfloat)normal.y;
        vertexBuffer[7].normal[2]   = (GLfloat)normal.z;
...
- (void)setSelected:(BOOL)flag
{
    ...
    if (flag == YES) {
        ...
        LDrawDragHandle *handle4 =
            [[[LDrawDragHandle alloc] initWithTag:4 position:self->vertex4] autorelease];
