...
- (id)initWithLines:(NSArray *)lines inRange:(NSRange)range parentGroup:(dispatch_group_t)parentGroup
{
    ...
    if (self) {
        ...
        while (lineIndex < [strippedLines count])
        {
            ...
            commandRange =
                [CommandClass rangeOfDirectiveBeginningAtIndex:lineIndex inLines:strippedLines maxIndex:[
                    strippedLines count] - 1];
            LDrawDirective *newDirective =
                [[CommandClass alloc] initWithLines:strippedLines inRange:commandRange parentGroup:parentGroup];
...
- (void)encodeWithCoder:(NSCoder *)encoder
{
    ...
    [encoder encodeBytes:(void *)&planePoint1 length:sizeof(Point3)
    forKey:@"planePoint1"];
    [encoder encodeBytes:(void *)&planePoint2 length:sizeof(Point3)
    forKey:@"planePoint2"];
    [encoder encodeBytes:(void *)&planePoint3 length:sizeof(Point3)
    forKey:@"planePoint3"];
} // end encodeWithCoder:
                ...
                for (counter = testRange.location + 1; counter < NSMaxRange(testRange); counter++) {
                    ...
                    if ([[self class] lineIsTextureBeginning:currentLine]) {
                        ...
                        NSRange nestedTextureRange =
                            [[self class] rangeOfDirectiveBeginningAtIndex:counter inLines:lines maxIndex:
                            NSMaxRange(testRange)];
    ...
    bestObject:(id *)bestObject bestDepth:(double *)bestDepth
{
    ...
    for (counter = 0; counter < commandCount; counter++) {
        currentDirective = [commands objectAtIndex:counter];
        [currentDirective depthTest:testPt inBox:bounds transform:transform creditObject:creditObject
        bestObject:bestObject bestDepth:bestDepth];
    }

    if (self->dragHandles) {
        for (LDrawDragHandle *handle in self->dragHandles) {
            [handle depthTest:testPt inBox:bounds transform:transform creditObject:creditObject bestObject:
...
- (NSString *)write
{
    ...
    // Start
    [written appendFormat:@"0 %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@", LDRAW_TEXTURE, LDRAW_TEXTURE_START,
    LDRAW_TEXTURE_METHOD_PLANAR, [LDrawUtilities outputStringForFloat:planePoint1.x],
    [LDrawUtilities outputStringForFloat:planePoint1.y], [LDrawUtilities outputStringForFloat:planePoint1.z],
    [LDrawUtilities outputStringForFloat:planePoint2.x], [LDrawUtilities outputStringForFloat:planePoint2.y],
    [LDrawUtilities outputStringForFloat:planePoint2.z], [LDrawUtilities outputStringForFloat:planePoint3.x],
    [LDrawUtilities outputStringForFloat:planePoint3.y], [LDrawUtilities outputStringForFloat:planePoint3.z],
    imageDisplayName];
...
- (void)setSelected:(BOOL)flag
{
    ...
    if (flag == YES) {
        LDrawDragHandle *handle1 =
            [[[LDrawDragHandle alloc] initWithTag:1 position:self->planePoint1] autorelease];
        LDrawDragHandle *handle2 =
            [[[LDrawDragHandle alloc] initWithTag:2 position:self->planePoint2] autorelease];
        LDrawDragHandle *handle3 =
            [[[LDrawDragHandle alloc] initWithTag:3 position:self->planePoint3] autorelease];
...
- (BOOL)parsePlanarTextureFromLine:(NSString *)line parentGroup:(dispatch_group_t)parentGroup
{
    ...
    @try
    {
        if ([scanner scanString:@"0" intoString:NULL] == NO) {
            @throw [NSException exceptionWithName:@"BricksmithParseException" reason:
                   @"Bad Planar TEXMAP syntax" userInfo:nil];
        }
    ...
    recursive
{
    ...
    if (recursive == YES) {
        ...
        for (LDrawDirective *directive in [self subdirectives]) {
            [directive flattenIntoLines:texLines triangles:texTriangles quadrilaterals:texQuadrilaterals other
            :texEverythingElse currentColor:parentColor currentTransform:transform normalTransform:
