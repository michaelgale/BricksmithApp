  ...
  hits:(NSMutableSet *)hits
{
  if (self->hidden == NO) {
    ...
    if (boundsOnly == NO) {
      if ([modelToDraw boxTest:bounds transform:combinedTransform boundsOnly:NO creditObject:creditObject hits
           :hits]) {
        if (creditObject != nil) {
          return(TRUE);
        }
  ...
  bestDepth:(double *)bestDepth
{
  if (self->hidden == NO) {
    ...
    [modelToDraw depthTest:pt inBox:bounds transform:combinedTransform creditObject:creditObject bestObject:
...
- (LDrawModel *) referencedMPDSubmodel
{
  ...
  if ((enclosingFile != nil) && (self->referenceName != nil)) {
  ...
  if (cacheType == PartTypeUnresolved) {
    ...
    if (mdpModel != nil) {
      ...
      if (cacheModel != nil) {
        ...
        cacheModel =
          [[ModelManager sharedModelManager] requestModel:referenceName withDocument:[self enclosingFile]];
        if (cacheModel) {
          ...
          [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(addedMPDModel:) name:
           LDrawMPDSubModelAdded object:nil];
        }
      }
    }
  }
